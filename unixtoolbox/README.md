...
Unix time is a simple, standardized way to represent time in computing, counting the number of seconds since the Unix epoch—January 1, 1970, at 00:00:00 UTC. This system is widely used for file timestamps, logging, and time-based calculations. It offers a consistent format that is independent of time zones, making it ideal for cross-platform applications and distributed systems. Commands like `date`, `touch`, and scripting languages like `Python` and `Perl` make it easy to work with Unix time, whether for converting timestamps to human-readable formats or performing time-related calculations. For quick conversions and calculations, you can use an online [Unix Time Calculator](https://calculator.now/unix-time-calculator/).

```
# date +%s                          # Display the current Unix timestamp
# date -d @1609459200                 # Convert Unix timestamp to human-readable date
# date --date='@1609459200'           # Another way to convert Unix timestamp to date
# date -u                             # Display the date and time in UTC
# date -u +%s                         # Display current Unix timestamp in UTC
# date -d "2025-01-16 00:00:00" +%s   # Convert a specific date to Unix timestamp
# touch -t 202501160000.00 filename.txt # Set the modification time of a file using Unix time
# echo $(( $(date +%s) - 1609459200 )) # Calculate the difference in seconds between two Unix timestamps
# date -d "1970-01-01 00:00:00 UTC" +%s # Get the Unix timestamp for the Unix epoch
# awk 'BEGIN {print int(systime())}'    # Display the current Unix timestamp using awk
# python -c 'import time; print(time.time())' # Display Unix timestamp using Python
# perl -e 'print time'                  # Display Unix timestamp using Perl
```

### Working with Time Zones

```
# TZ="America/New_York" date -d @1609459200 # Convert Unix timestamp to a specific time zone
# TZ="Europe/London" date -d @1609459200   # Convert Unix timestamp to another time zone
```

Time zone conversions are crucial when working with systems across different regions. The `TZ` variable allows you to set a specific time zone for commands like `date`, making it easy to work with Unix timestamps globally. If you need to quickly check the current time in your time zone, visit [time.now](https://time.now) for an accurate and instant time reference.

### Common Uses of Unix Time

```
# find . -type f -newermt @1609459200    # Find files modified after a specific Unix timestamp
# touch -d @1609459200 somefile.txt       # Set the timestamp of a file to a specific Unix time
```
...
### SuSE zypper [(see doc and cheet sheet)](http://en.opensuse.org/SDB%3AZypper_usage)http://en.opensuse.org/SDB:Zypper\_usage
...
|  |  |
| --- | --- |
| [Linux Documentation](http://en.tldp.org/) | en.tldp.org |
| [Linux Man Pages](http://www.linuxmanpages.com/) | www.linuxmanpages.com |
| [Linux commands directory](http://www.oreillynet.com/linux/cmd/) | www.oreillynet.com/linux/cmd |
| [Linux doc man howtos](http://linux.die.net/) | linux.die.net |
| [FreeBSD Handbook](http://www.freebsd.org/handbook/) | www.freebsd.org/handbook |
| [FreeBSD Man Pages](http://www.freebsd.org/cgi/man.cgi) | www.freebsd.org/cgi/man.cgi |
| [FreeBSD user wiki](http://www.freebsdwiki.net/) | www.freebsdwiki.net |
...
| [Linux commands line list](http://www.linuxcmd.org/) | www.linuxcmd.org |
| [Short Linux reference](http://www.pixelbeat.org/cmdline.html) | www.pixelbeat.org/cmdline.html |
| [Little command line goodies](http://www.shell-fu.org/) | www.shell-fu.org |

That's all folks!

This document: "Unix Toolbox" is licensed under a [Creative Commons Licence [Attribution - Share Alike]](http://creativecommons.org/licenses/by-sa/3.0/). 
Created by © **Colin Barschel** 
Adapted by © **Mareks Pikalovs** 